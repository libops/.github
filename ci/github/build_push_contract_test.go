...
func TestNativeBuildTimeoutRemainsFixed(t *testing.T) {
	workflow := workflowSource(t)
	inputs := stepBlock(t, workflow, "    inputs:\n", "    secrets:\n")
	buildHeader := stepBlock(t, workflow, "jobs:\n  build:\n", "    outputs:\n")

	requireContains(t, buildHeader, "timeout-minutes: 30")
	if strings.Contains(inputs, "build-timeout-minutes:") || strings.Contains(workflow, "BUILD_TIMEOUT_MINUTES") {
		t.Fatal("native build timeout must not be caller-controlled")
	}
}
